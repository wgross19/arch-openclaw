...
4. Confirm required paths:
   - `/mnt/user/appdata/openclaw` -> `/home/node/.openclaw`
   - `/mnt/user/appdata/openclaw/workspace` -> `/home/node/.openclaw/workspace`
5. Keep default network mode `bridge` and mapped UI port `18789`.
6. Start the container.

## First-Run Verification
1. Check container logs for successful gateway start.
2. Open UI:
   - `http://UNRAID-IP:18789/?token=YOUR_GATEWAY_TOKEN`
...
- Use host Tailscale IP or MagicDNS name with mapped UI port.
...
- Keep advanced root/chown options disabled unless repairing permissions.
