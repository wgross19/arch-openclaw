...
assert_contains "${DOCKERFILE}" "ARG NODE_VERSION=22\.13\.1"
assert_not_contains "${DOCKERFILE}" "ARG NODE_VERSION=22\.12\.0"
...
assert_contains "${DOCKERFILE}" "FROM \\$\\{CUDA_BASE_IMAGE\\} AS node-builder"
assert_contains "${DOCKERFILE}" "./configure --prefix=/usr/local --shared-sqlite"
...
assert_contains "${DOCKERFILE}" "libsqlite3-0"
assert_contains "${DOCKERFILE}" "sqlite_compileoption_used\('ENABLE_FTS5'\)"
assert_contains "${DOCKERFILE}" "create virtual table t using fts5\(x\)"
